...
) -> tuple[Any, Any] or tuple[Any, Tensor] or Any:
...
def _lpips_torch(raw_tensor: torch.Tensor, dst_tensor: torch.Tensor, crop_border: int) -> float:
    """PyTorch implements PSNR (Peak Signal-to-Noise Ratio, peak signal-to-noise ratio) function
    ...
    # Convert data type to torch.float32 bit
    raw_tensor = raw_tensor.to(torch.float32)
    dst_tensor = dst_tensor.to(torch.float32)
    
    lpips = LearnedPerceptualImagePatchSimilarity(net_type='vgg').to(torch.device("cuda", 0))
    
    raw_tensor = raw_tensor * 2 - 1
    dst_tensor = dst_tensor * 2 - 1
    
    lpips_metrics = lpips(raw_tensor, dst_tensor)
...
class LPIPS(nn.Module):
    ...
    def __init__(self, crop_border: int) -> None:
        super().__init__()
        self.crop_border = crop_border

    def forward(self, raw_tensor: torch.Tensor, dst_tensor: torch.Tensor) -> float:
        lpips_metrics = _lpips_torch(raw_tensor, dst_tensor, self.crop_border)
