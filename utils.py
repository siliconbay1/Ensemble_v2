...
from image_quality_assessment import PSNR, SSIM, LPIPS
...
def build_iqa_model(
    ...
    psnr_model = PSNR(crop_border=crop_border, only_test_y_channel=only_test_y_channel)
    ssim_model = SSIM(crop_border=crop_border, only_test_y_channel=only_test_y_channel)
    lpips_model = LPIPS(crop_border=crop_border)

    psnr_model = psnr_model.to(device)
    ssim_model = ssim_model.to(device)
    lpips_model = lpips_model.to(device)

    return psnr_model, ssim_model, lpips_model
...
def load_resume_state_dict(
        ...
        compile_state: bool,
        model_weights_path: str,
) -> tuple[Any, Module or None or Any, Any, Any, Any, Optimizer, Any] or tuple[Any, Module or None or Any, Any, Any, Any, Optimizer]:
