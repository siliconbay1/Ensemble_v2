...
def main(args):
    ...
    with torch.no_grad():
        ...
        residual_1 = sr_1 - bic_y_tensor
        residual_2 = sr_2 - bic_y_tensor
        map = torch.cat((sr_1.detach().clone(), sr_2.detach().clone()), 1)
        hypo = g_model(map)
        sr_res_tensor = bic_y_tensor + hypo[:,0:1,:,:] * residual_1 + hypo[:,1:2,:,:] * residual_2
    ...
    sr_y_image = imgproc.tensor_to_image(sr_res_tensor, range_norm=False, half=False)
    ...
    psnr = psnr_model(torch.clamp(sr_res_tensor, min=0, max=1), gt[:,0:1,:,:])
    ssim = ssim_model(torch.clamp(sr_res_tensor, min=0, max=1), gt[:,0:1,:,:])
    lpips = lpips_model(torch.cat((torch.clamp(sr_res_tensor, min=0, max=1), torch.clamp(bic_tensor[:,1:3,:,:], min=0, max=1)), 1), gt)
    ...
    # print(f"SR image save to `{prefix}_{args.upscale_factor}_{args.output_espcn_path}`")
    ...
    # print(f"SR image save to `{prefix}_{args.upscale_factor}_{args.output_fsrcnn_path}`")
    ...
    # sr_ycbcr_image = cv2.merge([sr_y_image[:, :, 0], bic_cb_image, bic_cr_image])
    # sr_image = imgproc.ycbcr_to_bgr(sr_ycbcr_image)
    # cv2.imwrite(args.output_vdsr_path, sr_image * 255.0)

    # print(f"SR image save to `{prefix}_{args.upscale_factor}_{args.output_vdsr_path}`")
