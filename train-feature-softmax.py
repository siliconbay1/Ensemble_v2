...
def main():
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        is_last = True #(epoch + 1) == config.epochs
...
def train(
    ...
    # for i in range(101): # it exists for memory capability test
        ...
        # vgg = vgg19.VGG(model_type='vgg11').to(device=config.device)
        # feature_1 = vgg(sr_1.to(device=config.device), ['relu1_1'])
        # feature_2 = vgg(sr_2.to(device=config.device), ['relu1_1'])
        # map = torch.cat((feature_1['relu1_1'], feature_2['relu1_1']), 1)
        
        # vgg = vgg19.VGG().to(device=config.device)
        # vgg.eval()
        # feature_1 = vgg(sr_1.to(device=config.device))
        # feature_2 = vgg(sr_2.to(device=config.device))        
        # map = torch.cat((feature_1, feature_2), 1)

        # hypo = F.softmax(unet(map), dim=1)
        # sr = sr_1 * hypo[:,0,:,:] + sr_2 * hypo[:,1,:,:]
